...
const CrearEspacio = () => {
  ...
  useEffect(() => {
	async function fetchTiposSubtipos() {
	  try {
		...
		const tipos = Array.from(new Set(espacios.map(e => e.tipo).filter(Boolean)));
		setTiposExistentes(tipos);
		// Si hay tipo seleccionado, filtrar subtipos de ese tipo
		if (tipo && tipo !== "nuevo") {
		  const subtipos = Array.from(new Set(espacios.filter(e => e.tipo === tipo).map(e => e.sub_tipo).filter(Boolean)));
  ...
  // Mostrar en consola los tipos de datos antes de enviar
  ...
  console.log('typeof precio:', typeof precio, 'typeof capacidad:', typeof capacidad);
  setLoading(true);
  try {
  ...
  return (
	<div className="landing" style={{ display: "flex", flexDirection: "column", minHeight: "100vh" }}>
	  ...
	  <Breadcrumbs />
  <div style={{ maxWidth: 1200, margin: "0 auto", width: "100%", padding: "0 20px", flex: 1, color: "#000" }}>
	...
	<div style={{ flex: 1, color: "#000" }}>
		...
		<div style={{ textAlign: "center" }}>
		  ...
		  <div style={{ display: "flex", justifyContent: "center", gap: 16 }}>
			...
			<button
			  onClick={() => {
				if (nuevoTipo) {
				  setTiposExistentes(prev => [...prev, nuevoTipo]);
				  setTipo(nuevoTipo);
				  setNuevoTipo("");
				  setModalTipoOpen(false);
	  ...
	  <Modal isOpen={modalSubTipoOpen} onClose={() => setModalSubTipoOpen(false)}>
		...
		{/* Carga de fotos (placeholder, deshabilitado) */}
	  <div style={{ flex: 1, opacity: 0.5, pointerEvents: "none", color: "#000" }}>
		<label style={{ display: "block", marginBottom: 8, fontWeight: 500, color: "#000" }}>Fotos del espacio</label>
		<div style={{ border: "2px dashed #ccc", borderRadius: 8, padding: 24, textAlign: "center", height: 200, display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", color: "#000" }}>
		  ...
		  <button style={{ background: "#e0e0e0", border: "none", borderRadius: 8, padding: "8px 16px", cursor: "pointer", marginTop: 8, color: "#000" }}>Explorar</button>
		</div>
	  </div>
	  </div>
